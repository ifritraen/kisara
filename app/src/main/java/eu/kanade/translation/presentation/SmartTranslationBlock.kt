...
fun SmartTranslationBlock(
    ...
    val translationPreferences = remember { uy.kohesive.injekt.Injekt.get<tachiyomi.domain.translation.TranslationPreferences>() }
    val maxLetters = remember { translationPreferences.maxLettersPerLine().get() }
    val fullCover = remember { translationPreferences.fullBoundingBoxCover().get() }
    val padXVal = remember { translationPreferences.paddingTextHorizontal().get().dp }
    val padYVal = remember { translationPreferences.paddingTextVertical().get().dp }
    val showWholeWord = remember { translationPreferences.showWholeWordEvenIfOutside().get() }

    val formattedText = remember(block.translation, maxLetters, showWholeWord) {
        if (showWholeWord) {
            block.translation
        } else {
            block.translation.split(" ").joinToString(" ") { word ->
                if (word.length > maxLetters) {
                    word.chunked(maxLetters).joinToString("\u00AD")
                } else {
                    word
                }
            }
    ...
    Box(
        ...
        val wordsToCheck = remember(formattedText) {
            formattedText.split(Regex("[\\s\\p{Punct}]+")).filter { it.length in 1..5 }
        }
        ...
        while (low <= high) {
            ...
            val wordTooWide = !showWholeWord && wordsToCheck.any { word ->
                textMeasurer.measure(text = word, style = midStyle).size.width > maxWidthPx
            }
            if (measured.size.height <= maxHeightPx && !wordTooWide) {
                bestSize = mid
                bestLayout = measured
                low = mid + 1
            } else {
        ...
        Canvas(
            ...
            val padX = padXVal.toPx()
            val padY = padYVal.toPx()
